...
@router.post("/text-to-summary-audio", response_model=SummarizationResponse)
async def convert_text_to_summary(request: TextSummarizationRequest):
    """Convert text to summary, then to audio file."""
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to generate summary and audio: {str(e)}")
